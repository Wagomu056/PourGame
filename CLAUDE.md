...
#### Current active screen: `GameScreen`

`main.ts` navigates `LoadScreen` → `GameScreen` (replaced the scaffold's `MainScreen`).

**`src/app/screens/game/GameScreen.ts`** — "Pour Beer!" PoC game screen.

- Implements the full 8-state game loop: `ARRIVING → WAIT_HOSE → FILLING → FULL → WAIT_CROWN → CROWN_PLACED → CAPPING → LEAVING → (loop)`
- Uses **PixiJS Graphics API only** — `assetBundles = []`, no texture assets. All visuals drawn with `Graphics.rect/circle/ellipse/poly/moveTo/lineTo` + `.fill()/.stroke()`.
- Rendering split: `bg: Graphics` (static background, redrawn in `resize()`) and `world: Graphics` (dynamic elements, cleared and redrawn in `update()` every frame).
- Input: pointer events on a transparent full-screen `hitSurface` + `window.addEventListener('keydown')` for `W` / `Space`. Always remove `keydown` listener in `hide()`.
- Beer drift formula: `sin(t × 1.6) × 50 + sin(t × 3.7) × 12 + random noise`. Hit zone = `NECK_W / 2` = 16 px.
